...
plt.minorticks_on()
plt.hist(MS_ad, bins=range(-200, 300, 20), label=r'$\rm 30\ Myr,\ \overline{v_{a}}$' + r'$={}$'.format(round(np.median(MS_ad),2)) + r'$\rm \ km \ s^{-1}$', density=True, histtype='step', linewidth=1.6,linestyle='--',stacked=True,fill=False, color='b')
plt.hist(AGy_ad,  bins=range(-200, 300, 20),label=r'$\rm 400\ Myr,\ \overline{v_{a}}$' + r'$={}$'.format(round(np.median(AGy_ad),2)) + r'$\rm \ km \ s^{-1}$',density=True, histtype='step', linewidth=1.6,stacked=True,fill=False, color='m', hatch='//', alpha=0.4)
plt.hist(AGo_ad,  bins=range(-200, 300, 20),label=r'$\rm 2\ Gyr,\ \overline{v_{a}}$' + r'$={}$'.format(round(np.median(AGo_ad),2)) + r'$\rm \ km \ s^{-1}$',density=True,histtype='step', linewidth=1.6,stacked=True,fill=True, color='green', alpha=0.4)
plt.hist(RG_ad, bins=range(-200, 300, 20), label=r'$\rm 4\ Gyr,\ \overline{v_{a}}$' + r'$={}$'.format(round(np.median(RG_ad),2)) + r'$\rm \ km \ s^{-1}$',density=True,histtype='step', linewidth=1.6,linestyle='-',stacked=True,fill=False, color='r')
plt.legend(loc=1, frameon=False, fontsize=10)
plt.xlim(-200,300)
plt.ylim(0, 0.021)
plt.xlabel(r'$ \rm Asymmetric\ Drift:\ \itv_{a}\ \rm(km\ s^{-1})$', fontsize=13)
plt.savefig('/Users/amandaquirk/Desktop/asymmetric_drift_zoom_poster.pdf')
...
axes[2].scatter(AGo_Av, AGo_dispersion, c='green',alpha=0.3)
axes[3].scatter(RG_Av, RG_dispersion, c='r',alpha=0.3)
axes[0].plot([0,5], [75, 75], linestyle='--', c='k')
axes[1].plot([0,5], [75, 75], linestyle='--', c='k')
axes[2].plot([0,5], [75, 75], linestyle='--', c='k')
axes[3].plot([0,5], [75, 75], linestyle='--', c='k')
