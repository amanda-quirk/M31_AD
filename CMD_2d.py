...
plt.hist(Av1, weights=weights1, bins=20, range=(0, 5),label=r'$\rm Region\ I$', normed=False, histtype='step', linewidth=1.6,stacked=True,fill=True, alpha=.4, color='b')
...
plt.hist(Av3, weights=weights3, bins=20, range=(0, 5), label=r'$\rm Region\ III$', normed=False, histtype='step', linewidth=1.6,linestyle='--',stacked=True,fill=False, color='r')
...
r1_med, vrot1_med = median_line(r1, vrot1)
r1_med, HI_vrot1_med = median_line(r1, HI_vrot1)
r1_clear_med, vrot1_clear_med = median_line(r1_clear, vrot1_clear)
r1_dust_med, vrot1_dust_med = median_line(r1_dust, vrot1_dust)
r1_clear_med, HI_vrot1_clear_med = median_line(r1_clear, HI_vrot1_clear)
r1_dust_med, HI_vrot1_dust_med = median_line(r1_dust, HI_vrot1_dust)
r2_med, vrot2_med = median_line(r2, vrot2)
r2_med, HI_vrot2_med = median_line(r2, HI_vrot2)
r3_med, vrot3_med = median_line(r3, vrot3)
r3_med, HI_vrot3_med = median_line(r3, HI_vrot3)

AD1_clear = asymmetric_drift(vrot1_clear, HI_vrot1_clear)
AD1_dust = asymmetric_drift(vrot1_dust, HI_vrot1_dust)
...
axes[0].scatter(r1_clear, vrot1_clear, s=4, c='r', alpha=0.4, label=r'$A_{v} \rm < 1$')
...
axes[0].annotate(r'$ \rm Region\ I$', xy=(11,143), horizontalalignment='left', fontsize=10)
axes[0].annotate(r'$\overline{v}_{\rm a, A_{v}<1}=$'+ '${}$'.format(round(np.median(AD1_clear),2)) + r'$\rm km\ s^{-1}$', xy=(11,128), horizontalalignment='left', fontsize=10)
axes[0].plot(r1_clear_med, vrot1_clear_med, linestyle='-', c='black', linewidth = 1.8, alpha=.85)
axes[0].plot(r1_clear_med, HI_vrot1_clear_med, linestyle='--', c='black', linewidth = 1.8, alpha=.85)
axes[0].plot(r1_dust_med, vrot1_dust_med, linestyle='-.', c='black', linewidth = 1.8)
axes[0].plot(r1_dust_med, HI_vrot1_dust_med, linestyle=':', c='black', linewidth = 1.8)
axes[1].plot(r2_med, vrot2_med, linestyle='-', c='black', linewidth = 1.8)
axes[1].plot(r2_med, HI_vrot2_med, linestyle='--', c='black', linewidth = 1.8)
axes[2].plot(r3_med, vrot3_med, linestyle='-', c='black', linewidth = 1.8)
axes[2].plot(r3_med, HI_vrot3_med, linestyle='--', c='black', linewidth = 1.8)
axes[1].annotate(r'$ \rm Region\ II$', xy=(19.2,128), horizontalalignment='right', fontsize=10)
axes[0].annotate(r'$\overline{v}_{\rm a, A_{v}\geq1}=$'+ '${}$'.format(round(np.median(AD1_dust),2)) + r'$\rm km\ s^{-1}$', xy=(11,112), horizontalalignment='left', fontsize=10)
axes[2].annotate(r'$ \rm Region\ III$', xy=(19.2,128), horizontalalignment='right', fontsize=10)
axes[1].annotate(r'$\overline{v}_{\rm a}=$'+ '${}$'.format(round(np.median(AD2),2)) + r'$\rm km\ s^{-1}$', xy=(19.2,112), horizontalalignment='right', fontsize=10)
axes[2].annotate(r'$\overline{v}_{\rm a}=$'+ '${}$'.format(round(np.median(AD3),2)) + r'$\rm km\ s^{-1}$', xy=(19.2,112), horizontalalignment='right', fontsize=10)
...
weights2 = np.ones_like(Av2)/float(len(Av2))
weights3 = np.ones_like(Av3)/float(len(Av3))
plt.hist(ad1, weights=weights1, bins=range(-100, 200, 15),label=r'$\rm Region\ I$',normed=False, histtype='step', linewidth=1.6,stacked=True,fill=True, alpha=0.4, color='b')
...
plt.hist(ad3, weights=weights3, bins=range(-100, 200, 15), label=r'$\rm Region\ III$',normed=False, histtype='step', linewidth=1.6,linestyle='--',stacked=True,fill=False, color='r')
plt.legend(loc=1, frameon=False)
plt.xlim(-100,200)
plt.xlabel(r'$\rm Asymmetric\ Drift:\ \it v_{a}\ \rm(km\ s^{-1})$', fontsize=13)
#plt.savefig('/Users/amandaquirk/Desktop/RGB_regions_ad.pdf')
plt.close()
